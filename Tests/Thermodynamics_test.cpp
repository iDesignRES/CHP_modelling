...
int main() {
  ...
  for (std::size_t np = 0; np < P.size(); np++) {
    std::cout << "P (bar-g) = " << P[np] << " , T sat (deg.C) = " << TSatWater(P[np])
              << std::endl;
    ...
    std::cout << "hPTSupSteam[kJ/kg] = " << hPTSupSteam(P[np], TK2) << std::endl;
    double h = hPTSupSteam(P[np], TK2);
    std::cout << "sPhSupSteam[kJ/kg*K] = " << sPhSupSteam(P[np], h) << std::endl;
    std::cout << "TPhSupSteam = " << TPhSupSteam(P[np], h) << std::endl;
    std::cout << "hPSatSteam[kJ/kg] = " << hPSatSteam(P[np]) << std::endl;
    std::cout << "sPTSupSteam[kJ/kg*K] = " << sPTSupSteam(P[np], TK2) << std::endl;
