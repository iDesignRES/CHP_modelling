...
void NOx_reduction_model(flow &in, flow &out, vector<flow> &cons, vector<parameter> &par) {
  ...
  double X_NOx_clean =
      atof(get_parameter_value(par, "process", "AIR_EMISSIONS", "NOx").c_str());
  ...
  cons[ammonia].F.P =
      atof(get_parameter_value(par, "process", "NOx_REDUCTION", "P_NH3").c_str());
  ...
  export_parameter(par, "output", "process", "NOx_REDUCTION", "M_NH3",
                   to_string(cons[ammonia].F.M));
}
